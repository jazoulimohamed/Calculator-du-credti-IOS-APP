//
//  BorrowingCapacityViewController.m
//  Immofinances
//
...
@interface BorrowingCapacityViewController ()
...
@implementation BorrowingCapacityViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.amountStepValue = 100.0f;
    // Set the initial value to prevent any weird inconsistencies.
    self.lastAmountStep = 750 / self.amountStepValue;
...
- (void)initializeComponents {
    ...
    self.amountSlider.minimumValue = 200.0;
    self.amountSlider.maximumValue = 1500.0;
    self.amountSlider.value = 750.0;
...
- (void)evaluateTotal {
    ...
    NSString *error = @"";
    if ((borrowDuration <= 0 || borrowDuration > 40)) {
    ...
    if ([error isEqualToString:@""]) {
        ...
        double pret  = borrowedAmount / t;
        [self.totalTextField setText:[NSString stringWithFormat:@"%.0f", pret]];
    } else {
...
- (IBAction)returnToHomeViewFromBorrowingCapacityView:(id)sender {
    [self.delegate returnToHomeViewFromBorrowingCapacityView];
}
